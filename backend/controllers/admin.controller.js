...
const sanitizeCSV = (val) => {
  ...
  // If it starts with a formula character, prefix with a quote
...
const downloadCSV = async (req, res) => {
  try {
   ...
   rows.forEach(r => {
  csvRows.push([
    sanitizeCSV(r.nickname),
    sanitizeCSV(r.session_name),
...
const inviteAdmin = async (req, res) => {
  ...
  try {
    ...
    // Send invite email
    const inviteLink = `${process.env.ADMIN_URL}/admin/register?token=${token}`;
    try {
